...
export default async function handler(req, res) {
  ...
  if (parts.length === 0) { res.writeHead(302, { Location: '/index.html' }); return res.end(); }

  const username = parts[0];
  try {
    const userRes = await pool.query('SELECT id, username FROM users WHERE LOWER(username)=$1', [username]);
    if (userRes.rows.length === 0) { res.status(404).send('<h1>User not found</h1>'); return; }
...
<style>body{margin:0;overflow:hidden;font-family:Inter,sans-serif;}.profile-canvas{position:relative;width:100vw;height:100vh;}</style>
</head>
<body>${clickEnterHTML}<div class="profile-canvas">${widgetsHTML}</div></body>
</html>`;
