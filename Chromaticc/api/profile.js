...
function renderWidget(widget) {
  ...
  switch (widget.type) {
    ...
    case 'audio':
      return `<div style="${style} display:flex; flex-direction:column; justify-content:center;"><strong>${esc(s.title)}</strong><audio controls src="${esc(s.src)}" style="width:100%;"></audio></div>`;
    case 'youtube':
    ...
    case 'glass-toggle':
      // This is a global toggle, but can be rendered as a note
  ...
  try {
    ...
    if (settings.background) {
      ...
      if (bg.type === 'image' || bg.type === 'video') {
        globalStyles += `body{background:url('${bg.value}') center/cover fixed;}`;
      } else {
